...
const UsersList = ({ users }: UsersListProps) => {
  ...
  const handleDelete = () => {
    if (contextMenu) {
      setDeleteModal({ open: true, user: contextMenu.user });
    }
    setContextMenu(null);
  };
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        router.refresh(); // رفرش لیست بعد از حذف موفق
      } else {
    ...
    <>
      ...
      <Modal
        ...
        actions={[
          ...
          {
            label: isDeleting ? 'در حال حذف...' : 'بله، حذف شود',
            variant: 'danger',
      ...
      >
        <p>
          آیا از حذف کاربر{' '}
          <span className="font-extrabold text-gray-800">{deleteModal.user?.fullName}</span> مطمئن
          هستید؟
        </p>
